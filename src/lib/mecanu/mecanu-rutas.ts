/* Mecanu — MODELO DE LOGÍSTICA.
   ...
   Este módulo re-exporta todo mecanu-data y mecanu-pipeline: el panel importa solo este.
...
export * from './mecanu-data';
export * from './mecanu-pipeline';

import {
  at, TALLER, cliente, vehiculo, conductor,
  servicio, INSPECCIONES_RAW, ZONAS_CARROCERIA, foto, OPORTUNIDADES_BASE, DIAS_SEMANA,
  localizarDireccion,
} from './mecanu-data';

import {
  subestadoMeta, etiquetaParada, etiquetaPaso, PARADA_SUBTIPOS, ROLES_TRAMO,
  INDICADOR_TRAMO, TAGS_DERIVADOS, TAGS_MANUALES, PRESUPUESTO_PENDIENTES,
  DIAS_CADUCIDAD_OFERTA, SERVICIO_TRASLADO_ID, seComunicaAlCliente,
  type TagCtx, type TagManualCfg,
} from './mecanu-pipeline';

import {
  CampanaSchema, ParadaSchema, PresupuestoSchema, RutaSchema, TramoSchema,
  type Actividad, type Campana, type CampanaItem, type EstadoRuta, type EstadoTramo,
  type Inspeccion, type LineaPresupuesto, type Log, type LogTipo, type OrigenLinea,
  type Parada, type ParadaSubtipo, type ParadaTipo, type Presupuesto, type PresupuestoEstado,
...
export const PROVEEDORES: Record<ParadaSubtipo, string> = {
...
const dirCliente = (id: string): string | null => { const c = cliente(id); return c ? c.direccion : null; };
const franjaStr = (v: Ventana | null | undefined): string | null => (v && v.inicio ? `${v.inicio} - ${v.fin}` : null);
const horaNum = (hhmm: string): number => Number(String(hhmm).split(':')[0]);
const fechaEn = (dia: number, hhmm: string): Date => at(horaNum(hhmm), Number(String(hhmm).split(':')[1] || 0), dia);
...
/** Parada tal y como se especifica aquí arriba: la `Parada` real se deriva de esto. */
export interface ParadaRaw {
  tipo: ParadaTipo;
  subtipo: ParadaSubtipo | null;
  /** `undefined` = se resuelve sola (dirección del cliente o del proveedor); `null` = sin dirección */
  direccion?: string | null;
  /** descripciones de los servicios que se hacen en esta parada */
  servicios?: string[];
}

/** Ventana tentativa de un prospecto, en el formato compacto de la especificación. */
export interface FranjaRaw { dia: number; franja: string }

/** Tramo tal y como se especifica aquí arriba: el `Tramo` real se deriva de esto. */
export interface TramoRaw {
  rol: RolTramo;
  conductorId: string | null;
  /** offset en días respecto de HOY; null = sin agenda */
  dia: number | null;
  /** "HH:MM - HH:MM"; null = sin agenda */
  franja: string | null;
  seguro: boolean;
  estado: EstadoTramo;
  modo: VentanaModo;
...
/** Línea de presupuesto añadida a mano a la ruta o a la campaña. */
...
/** Especificación de una ruta. Es el único sitio donde se escriben datos a mano:
    RUTAS · PARADAS · TRASLADOS · LOGS · PRESUPUESTOS se derivan todos de aquí. */
export interface RutaRaw {
  id: string;
  clienteId: string | null;
  vehiculoId: string | null;
  /** matrícula de un lead sin ficha de cliente todavía */
  matriculaLead?: string;
  perfil: string;
  estado: EstadoRuta;
  subestado: string;
  /** descripción del servicio del taller (da nombre a la línea de reparación) */
  servicio: string;
  /** importe de la reparación, sin los traslados; 0 = todavía sin valorar */
  reparacion: number;
  presEstado: PresupuestoEstado;
  presModo?: PresupuestoModo;
  lineasExtra?: LineaRaw[];
  tagsManual?: string[];
  clienteTieneAuto?: boolean | null;
  vehiculoListo?: boolean | null;
  campanaOrigenId?: string;
  motivo?: string;
  incidencia?: string;
  cancelada?: { dia: number; hora: string };
  link?: { token: string; dia: number; hora: string };
  paradas: ParadaRaw[];
  tramos: TramoRaw[];
}

const P = (tipo: ParadaTipo, subtipo: ParadaSubtipo | null, extra?: Partial<ParadaRaw>): ParadaRaw =>
  Object.assign({ tipo, subtipo: subtipo || null }, extra || {});
const cli = (extra?: Partial<ParadaRaw>): ParadaRaw => P('cliente', null, extra);
const prov = (subtipo: ParadaSubtipo, extra?: Partial<ParadaRaw>): ParadaRaw => P('proveedor', subtipo, extra);

const RUTAS_RAW: RutaRaw[] = [
  /* ----- MIGRADAS ----- */
...
let logSeq = 0;
function log(
  trasladoId: string, tipo: LogTipo, ts: Date, actor: string,
  triggerSource: TriggerSource, payload?: Log['payload'],
): Log {
...
function linea(descripcion: string, importe: number, origen: OrigenLinea, servicioTemparioId?: string | null): LineaPresupuesto {
  return { descripcion, importe, origen, servicioTemparioId: servicioTemparioId || null };
}

/** Todo lo que no venga en el spec cae a su valor por defecto; `total` lo calcula la propia función. */
export type PresupuestoSpec = Partial<Omit<Presupuesto, 'id' | 'total'>>;

function crearPresupuesto(id: string, spec: PresupuestoSpec): Presupuesto {
  ...
  } as Omit<Presupuesto, 'total'>, spec) as Presupuesto;
...
RUTAS_RAW.forEach((raw) => {
  ...
  /* — PARADAS — */
  const paradas: Parada[] = raw.paradas.map((p, i) => {
  ...
  /* — TRASLADOS (tramos) — */
  const tramos: Tramo[] = raw.tramos.map((t, i) => {
  ...
  tramos.forEach((t) => {
    ...
    const menos = (min: number) => new Date(base.getTime() - min * 60000);
    const mas = (min: number) => new Date(base.getTime() + min * 60000);
    ...
    if (ruta.linkToken && t.orden === 1) {
      /* `linkEnviadoEn!`: si hay token hay fecha de envío — ambos salen de `raw.link`. */
...
/* ── Contrato del modelo: lo construido cumple los schemas de types.ts ── */
ParadaSchema.array().parse(PARADAS);
TramoSchema.array().parse(TRASLADOS);
RutaSchema.array().parse(RUTAS);

/* ==============================================================
   3 · ACCESO Y DERIVACIONES
   ============================================================== */

export const ruta = (id: string | null | undefined): Ruta | null => RUTAS.find((r) => r.id === id) || null;
export const rutaVista = (id: string | null | undefined): RutaVista | null => { const r = ruta(id); return r ? vistaRuta(r) : null; };
export const parada = (id: string | null | undefined): Parada | null => PARADAS.find((p) => p.id === id) || null;
export const tramo = (id: string | null | undefined): Tramo | null => TRASLADOS.find((t) => t.id === id) || null;
export const presupuesto = (id: string | null | undefined): Presupuesto | null => PRESUPUESTOS.find((p) => p.id === id) || null;

export const paradasDeRuta = (rutaId: string): Parada[] => PARADAS.filter((p) => p.rutaId === rutaId).sort((a, b) => a.orden - b.orden);
export const tramosDeRuta = (rutaId: string): Tramo[] => TRASLADOS.filter((t) => t.rutaId === rutaId).sort((a, b) => a.orden - b.orden);
export const logsDeTramo = (tramoId: string): Log[] => LOGS.filter((l) => l.trasladoId === tramoId);
export const logsDeRuta = (rutaId: string): Log[] => {
  const ids = tramosDeRuta(rutaId).map((t) => t.id);
  return LOGS.filter((l) => ids.indexOf(l.trasladoId) >= 0);
};

export const rutasDeCliente = (id: string): RutaVista[] => RUTAS.filter((r) => r.clienteId === id).map(vistaRuta);
export const rutasDeVehiculo = (id: string): RutaVista[] => RUTAS.filter((r) => r.vehiculoId === id).map(vistaRuta);
export const rutasDeConductor = (id: string): RutaVista[] => {
...
const ESTADOS_ABIERTOS: EstadoRuta[] = ['prospectos', 'agendado', 'en_ruta', 'en_taller'];
export const rutaAbiertaDeVehiculo = (vehiculoId: string | null, excluirId?: string): Ruta | null =>
...
export function tramoActivo(rutaId: string): Tramo | null {
...
/** Dónde está el vehículo ahora mismo, derivado de su ruta abierta. */
export interface UbicacionVehiculo {
  ubicacionActual: 'con_cliente' | 'en_viaje' | 'en_parada';
  paradaActualId: string | null;
  rutaId: string | null;
}

/** Derivado: dónde está el vehículo. */
export function ubicacionVehiculo(vehiculoId: string): UbicacionVehiculo {
...
/** Contacto de un vehículo: derivado, nunca persistido (ver CLAUDE.md, decisión 8). */
export interface ContactoVehiculo {
  clienteId: string;
  nombre: string;
  rol: 'titular' | 'otro';
  relacion: string;
...
export function contactosDeVehiculo(vehiculoId: string): ContactoVehiculo[] {
...
export function tagsDeRuta(r: Ruta | null | undefined, ahora?: number): TagRuta[] {
  if (!r) return [];
  ...
  const derivados: TagRuta[] = TAGS_DERIVADOS.filter((t) => {
    try { return !!t.calc(ctx); } catch { return false; }
  }).map((t) => ({ id: t.id, label: t.label, color: t.color, derivado: true }));
  /* `filter(Boolean)` limpia los ids que ya no existen en el catálogo; TS no lo estrecha solo. */
  const manuales: TagRuta[] = ((r.tagsManual || []).map((id) => TAGS_MANUALES.find((t) => t.id === id))
    .filter(Boolean) as TagManualCfg[])
    .map((t) => ({ id: t.id, label: t.label, color: t.color, emoji: t.emoji || '', derivado: false }));
  return derivados.concat(manuales);
}

/* -- Indicador de tramo activo y trayecto de la card -- */

/** Indicador del tramo activo tal y como lo pinta la card del kanban. */
export interface TramoActivoVista {
  label: string;
  icono: string;
  origen: string;
  destino: string | null;
  trayecto: string;
  /** true = el coche no está viajando, reposa en una parada */
  esReposo: boolean;
}

export function tramoActivoVista(r: Ruta | null | undefined): TramoActivoVista | null {
  if (!r) return null;
  const activo = tramoActivo(r.id);
...
export function pasosDeRuta(rutaId: string): string[] {
  const ps = paradasDeRuta(rutaId);
  return ps.map((p, i) => etiquetaPaso(p, i, ps.length));
}

export function pasoActualDeRuta(r: Ruta): number {
  const ps = paradasDeRuta(r.id);
...
/* Una sola firma pública —con una ruta real, siempre hay vista— sobre una implementación
   que sigue aceptando null y devolviendo null, tal cual estaba. Así `RUTAS.map(vistaRuta)`
   es `RutaVista[]` sin castings y la guarda defensiva se mantiene en runtime. */
export function vistaRuta(r: Ruta): RutaVista;
export function vistaRuta(r: Ruta | null | undefined): RutaVista | null {
  if (!r) return null;
  const activo = tramoActivo(r.id);
  const tramos = tramosDeRuta(r.id);
...
export function descripcionServicioDeRuta(rutaId: string): string {
  const servicios = paradasDeRuta(rutaId).reduce<string[]>((acc, p) => acc.concat(p.servicios.map((s) => s.descripcion)), []);
  return servicios.length ? servicios.join(' · ') : '—';
}

/** Las rutas tal y como las consume el panel (tabla, kanban, KPIs). */
export const RUTAS_VISTA: RutaVista[] = RUTAS.map(vistaRuta)
...
export function actividadDeRuta(rutaId: string): Actividad[] {
...
export function inspeccionesDeRuta(rutaId: string | null | undefined): Inspeccion[] {
  /* Sin ruta no hay inspecciones: antes se llegaba al mismo [] pasando por un lookup
     fallido y un map sobre un array vacío. */
  if (!rutaId) return [];
  const r = ruta(rutaId);
...
/** Estado y vínculos con los que se completa cada oportunidad hasta ser una `Campana`. */
interface CampanaMeta {
  ...
  lineasExtra?: LineaRaw[];
  /** añade la línea de traslado (ida y vuelta) al presupuesto de la campaña */
  conTraslado?: boolean;
}

const CAMPANA_META: Record<string, CampanaMeta> = {
...
/** Campaña escrita a mano (no nace de una oportunidad detectada en una inspección). */
interface CampanaExtraRaw {
  id: string;
  clienteId: string;
  vehiculoId: string;
  estado: PresupuestoEstado;
  modo: PresupuestoModo;
  rutaOrigenId: string | null;
  rutaGeneradaId?: string;
  inspeccionId?: string;
  falla: string;
  fotoSeed: string;
  ultimo: { dow: number; hora: number };
  lineas: LineaRaw[];
}

/* Campañas nuevas que cubren los estados y los casos que faltaban. */
const CAMPANAS_EXTRA: CampanaExtraRaw[] = [
...
function lineasDeOportunidad(o: { items?: CampanaItem[] }, meta: CampanaMeta): LineaPresupuesto[] {
...
export const CAMPANAS: Campana[] = OPORTUNIDADES_BASE.map((o): Campana => {
...
}).concat(CAMPANAS_EXTRA.map((c): Campana => {
...
/* Contrato: campañas y presupuestos cumplen su schema. Va aquí, con CAMPANAS ya cerrado,
   porque `crearPresupuesto()` sigue añadiendo a PRESUPUESTOS hasta esta línea. */
CampanaSchema.array().parse(CAMPANAS);
PresupuestoSchema.array().parse(PRESUPUESTOS);

/** Alias histórico: el pipeline de Campañas trabaja con estas mismas cards. */
export const OPORTUNIDADES: Campana[] = CAMPANAS;

export const campana = (id: string | null | undefined): Campana | null => CAMPANAS.find((c) => c.id === id) || null;
export const oportunidadesPorTipo = (tipo: string): Campana[] =>
...
export const campanasPendientes = (lista?: Campana[]): number =>
...
export function campanaDesdeInspeccion(insp: Inspeccion, rutaId: string): Campana | null {
  ...
  const p = crearPresupuesto(`PR-${id}`, {
    ...
    /* `h.servicio!`: el filtro de arriba ya deja solo hallazgos con servicio. */
