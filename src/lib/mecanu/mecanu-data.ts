/* Mecanu — capa de datos en memoria (frontend only).
   ...
   El modelo de logística (RUTA · PARADA · TRASLADO · LOG · PRESUPUESTO) vive en mecanu-rutas.
   Relación cliente ↔ vehículo: muchos-a-muchos (tabla `usuarios` dentro de cada vehículo).

   Las formas de las entidades NO se declaran aquí: se importan de `types.ts`, que es la
   fuente única de verdad (ver AGENTS.md). Al final de cada array construido hay un
   `.parse()` del schema Zod correspondiente: si el dato deriva del contrato, revienta al
   cargar el módulo, no tres pantallas más adelante. */

import {
  CampanaItemSchema, ClienteSchema, ConductorSchema, ServicioSchema, VehiculoSchema,
  type Campana, type CampanaItem, type Cliente, type Conductor, type DanoInspeccion,
  type HallazgoInspeccion, type RutaVista, type Servicio,
  type SeveridadInspeccion, type TipoInspeccion, type Vehiculo,
} from './types';

export const HOY = new Date();
HOY.setSeconds(0, 0);

export function at(h: number, m: number, offsetDias = 0): Date {
...
export function fmtDia(d: Date | null | undefined): string {
  if (!d) return '—';
...
export function fmtDiaHora(d: Date | null | undefined): string {
  return d ? `${fmtDia(d)} · ${fmtHora(d)}` : '—';
}
export function fmtDinero(n: number | null | undefined, sinDecimales?: boolean): string {
  if (n == null) return '—';
  // useGrouping se fuerza explícitamente: en algunos entornos Node/ICU el separador
  // de miles de es-ES con style:'currency' no se aplica por defecto ("1234,56 €" en
  // vez de "1.234,56 €") aunque sí lo hace con useGrouping true — comprobado en vivo,
  // no es una suposición. Dinero siempre debe llevar el separador (regla dura, CLAUDE.md).
  const opts: Intl.NumberFormatOptions = { style: 'currency', currency: 'EUR', useGrouping: true };
...
export function maskTel(t: string | null | undefined): string {
  return t ? t.slice(0, 3) + ' ••• •••' : '—';
}
/** RGPD: dirección aproximada (calle sin número ni piso) para vistas de listado. */
export function maskDireccion(dir: string | null | undefined): string {
  if (!dir) return '—';
...
export function nombreCliente1(nombre: string | null | undefined): string {
  if (!nombre) return '—';
  if (/\b(S\.?\s?L\.?U?|S\.?\s?A\.?|C\.?\s?B\.?)\b/i.test(nombre)) return nombre;
  const p = nombre.trim().split(/\s+/);
...
const BARRIOS_BCN: Record<string, string> = {
...
export function localizarDireccion(dir: string | null | undefined): { sublocalidad: string | null; localidad: string | null } {
...
/* Los estados, subestados, columnas del kanban y tags viven en mecanu-pipeline
   (config declarativa). Aquí solo queda el onboarding de conductores, que es otro pipeline. */

export const ONBOARDING_META: Record<string, { label: string; kind: string; paso: number; desc: string }> = {
...
export const ORDEN_ONBOARDING: string[] = ['documentos_pendientes', 'en_supervision', 'activo'];
...
/* -- Contrato: los datos base cumplen su schema o el módulo no carga -- */
ClienteSchema.array().parse(CLIENTES);
VehiculoSchema.array().parse(VEHICULOS);
ConductorSchema.array().parse(CONDUCTORES);

/* Relaciones base. Las rutas, paradas, traslados y logs se construyen en mecanu-rutas. */
export const cliente = (id: string | null | undefined): Cliente | null => CLIENTES.find(c => c.id === id) || null;
export const vehiculo = (id: string | null | undefined): Vehiculo | null => VEHICULOS.find(v => v.id === id) || null;
export const conductor = (id: string | null | undefined): Conductor | null => CONDUCTORES.find(d => d.id === id) || null;

/** Vehículo + qué relación tiene con él el cliente por el que se preguntó. */
export type VehiculoDeCliente = Vehiculo & { relacion: string };

export const vehiculosDeCliente = (clienteId: string): VehiculoDeCliente[] =>
  VEHICULOS.filter(v => v.usuarios.some(u => u.clienteId === clienteId))
    /* el `!` es seguro: el filter de arriba garantiza que el usuario existe */
    .map(v => ({ ...v, relacion: v.usuarios.find(u => u.clienteId === clienteId)!.relacion }));

/** Cliente + su relación con el vehículo por el que se preguntó. */
export type ClienteDeVehiculo = Partial<Cliente> & { relacion: string; principal: boolean };

export const clientesDeVehiculo = (vehiculoId: string | null | undefined): ClienteDeVehiculo[] => {
  const v = vehiculo(vehiculoId);
  return v ? v.usuarios.map(u => ({ ...cliente(u.clienteId), relacion: u.relacion, principal: u.principal })) : [];
};

export const etiquetaVehiculo = (v: Pick<Vehiculo, 'marca' | 'modelo'> | null | undefined): string =>
  (v ? v.marca + ' ' + v.modelo : '—');
...
const firma = (path: string): string => 'data:image/svg+xml;utf8,' + encodeURIComponent(
...
export const foto = (seed: string): string => `https://picsum.photos/seed/${seed}/900/620`;

export const ZONAS_CARROCERIA: string[] = [
...
export const SEVERIDAD_META: Record<SeveridadInspeccion, { kind: string; label: string }> = {
  ok:      { kind: 'positive', label: 'Correcto' },
  warning: { kind: 'warning',  label: 'Observación' },
  danger:  { kind: 'alert',    label: 'Crítico' },
};

/** Daño tal y como lo registra el conductor: sin `fotoUrl`, que se deriva al leer. */
export type DanoRaw = Omit<DanoInspeccion, 'fotoUrl'>;
/** Hallazgo tal y como lo registra el conductor: sin `fotoUrl`, que se deriva al leer. */
export type HallazgoRaw = Omit<HallazgoInspeccion, 'fotoUrl'>;

/** Inspección en crudo. `inspeccionesDeRuta()` (mecanu-rutas) la completa hasta `Inspeccion`:
    resuelve fecha, tramo, vehículo, sede y las URLs de las fotos. */
export interface InspeccionRaw {
  id: string;
  tipo: TipoInspeccion;
  /** [hora, minuto] relativo al día `dia` */
  fechaH: [number, number];
  /** offset en días respecto de HOY */
  dia: number;
  /** id del conductor que la firma */
  inspector: string;
  vin: string;
  km: number;
  combustible: string;
...
/** Clave = id de ruta. */
export const INSPECCIONES_RAW: Record<string, InspeccionRaw[]> = {
  'TR-1039': [
...
export function normalizePlate(str: string | null | undefined): string {
...
export function buscarMatricula(q: string): { exactas: Vehiculo[]; sugerida: Vehiculo | null } {
...
/** `ok` es truthy/falsy, no estrictamente booleano: la última rama devuelve el match del
    geocoder simulado (así se conserva el comportamiento original, que solo lo evalúa). */
export function verificarDireccion(
  texto: string,
): { ok: boolean | RegExpMatchArray | null; motivo?: string; normalizada?: string } {
...
/* ======================================================================
   ...
   ====================================================================== */
export const TALLER: { nombre: string; direccion: string } =
  { nombre: 'Talleres Rodríguez', direccion: 'Carrer de Numància 105, Nau 3, 08029 Barcelona' };

/** Pool de direcciones reales (Barcelona / Cataluña) que el geocoder da por verificadas. */
export const DIRECCIONES_POOL: string[] = [
  TALLER.direccion,
...
function sinAcentos(s: string | null | undefined): string { return (s || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, ''); }

/** Sugerencias de direcciones filtradas por texto (como el autocompletar de Maps). */
export function sugerirDirecciones(q: string, limit = 6): string[] {
  const norm = sinAcentos(q).trim();
  if (norm.length < 2) return [];
  const tokens = norm.split(/\s+/).filter(Boolean);
  const pool = Array.from(new Set(DIRECCIONES_POOL.concat(CLIENTES.map((c) => c.direccion))));
  /* `filter(Boolean)` descarta los nulls en runtime pero no estrecha el tipo: se afirma aquí. */
  ...
  }).filter(Boolean) as { d: string; score: number }[]).sort((a, b) => a.score - b.score);
  return puntuadas.slice(0, limit).map((p) => p.d);
}

export function parseFranja(f: string): { start: number; end: number } {
...
export function franjasConflictan(f1: string, f2: string): boolean {
  const a = parseFranja(f1), b = parseFranja(f2);
  return !((a.end + 1 <= b.start) || (b.end + 1 <= a.start));
}
export const FRANJAS: string[] = ['08:00 - 09:00','09:00 - 10:00','10:00 - 11:00','11:00 - 12:00','12:00 - 13:00','13:00 - 14:00','16:00 - 17:00','17:00 - 18:00','18:00 - 19:00'];

/** Conflicto de agenda: busca traslados activos del mismo conductor el mismo día que se solapen (+1h de margen). */
export function conflictoConductor(
  todasLasRutas: RutaVista[],
...
export function historialInspeccionesVehiculo(vehiculoId: string): Date[] {
...
export function labelRegistro(idx: number): string {
...
export const CATEGORIAS_SERVICIO: string[] = ['Iluminación', 'Neumáticos', 'Frenos', 'Mantenimiento', 'Estética', 'Traslado'];

/** Servicio del tempario antes de calcular sus totales. */
export type ServicioRaw = Omit<Servicio, 'total' | 'totalIva'>;

const SERVICIOS_RAW: ServicioRaw[] = [
...
export const SERVICIOS: Servicio[] = SERVICIOS_RAW.map((s) => {
  const total = s.manoObra + s.materiales;
  return Object.assign({}, s, { total, totalIva: Math.round(total * (1 + IVA) * 100) / 100 });
});

ServicioSchema.array().parse(SERVICIOS);

export const servicio = (id: string | null | undefined): Servicio | null => SERVICIOS.find((s) => s.id === id) || null;
export const fmtHoras = (h: number): string => `${String(Math.floor(h)).padStart(2, '0')}:${String(Math.round((h % 1) * 60)).padStart(2, '0')} h`;

/* ======================================================================
   CAMPAÑAS — oportunidades detectadas en las inspecciones visuales
   ====================================================================== */

export const TIPOS_CAMPANA: { id: string; label: string; icon: string }[] = [
...
export const CATALOGO_DETECCION: Record<string, { etiqueta: string; servicioId: string }> = {
...
export const DIAS_SEMANA: string[] = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'];

/** Próxima fecha que repite el hábito del cliente (mismo día de la semana y hora). */
function proximoHabito(dow: number, hora: number, minDias: number): Date {
  const d = new Date(HOY);
...
/** Ítem de campaña antes de resolverlo contra el catálogo/tempario. */
export type CampanaItemRaw = Omit<CampanaItem, 'id' | 'etiqueta' | 'servicio' | 'valor' | 'fecha'>;

/** Hábito del cliente: día de la semana (0 = domingo) y hora del último servicio. */
export interface HabitoCliente { dow: number; hora: number }

export interface OportunidadRaw {
  id: string;
  clienteId: string;
  vehiculoId: string;
  inspeccionId: string | null;
  fotoSeed: string;
  ultimo: HabitoCliente;
  items: CampanaItemRaw[];
}

const OPORTUNIDADES_RAW: OportunidadRaw[] = [
...
/** Oportunidad ya resuelta: es una `Campana` a falta de su presupuesto y de sus vínculos
    con rutas, que les pone `CAMPANAS` en mecanu-rutas. */
export type OportunidadBase =
  Omit<Campana, 'rutaOrigenId' | 'rutaGeneradaId' | 'presupuestoId' | 'presupuesto' | 'estado' | 'origenAutomatico'>
  & { fotoSeed: string; ultimo: HabitoCliente };

export const OPORTUNIDADES_BASE: OportunidadBase[] = OPORTUNIDADES_RAW.map((o) => {
  const items: CampanaItem[] = o.items.map((it, i) => {
    const cat: Partial<{ etiqueta: string; servicioId: string }> = CATALOGO_DETECCION[it.tipo] || {};
...
/* Los ítems ya son entidades completas: se validan contra su schema. La campaña entera
   no puede validarse todavía — le falta el presupuesto, que se le añade en mecanu-rutas. */
CampanaItemSchema.array().parse(OPORTUNIDADES_BASE.flatMap((o) => o.items));
