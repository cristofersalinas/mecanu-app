...
export function e164(tel, prefijo = '34') {
...
export function fromISO(s) { return new Date(s + 'T00:00:00'); }
export function rangoFecha(base) {
...
export function validarVariable(valor) {
...
export const frase = (etiqueta) => FRASE_SERVICIO[etiqueta] || etiqueta.toLowerCase();

export const ETIQUETA_ORIGEN = { confirmado: 'Confirmado', estimado: 'Sugerido' };
...
export function inspeccionDe(o) {
...
function buscarHallazgo(insp, it) {
...
const compact = (arr) => arr.filter((x) => x != null && x !== '');
...
const medidaNeumatico = (sv) => { const m = sv && sv.nombre.match(/(\d{3}\/\d{2}\s?R\d{2})/); return m ? m[1] : null; };
const capacidadBateria = (sv) => { const m = sv && sv.nombre.match(/(\d+\s?Ah)/i); return m ? m[1] : null; };
const tipoAceiteCatalogo = (sv) => (sv && sv.notas && /aceite/i.test(sv.notas)) ? sv.notas : null;
...
export function detalleHallazgo(o, it) {
...
export function valoresOportunidad(o, seleccion, overrides) {
  const ov = overrides || {};
...
export function renderMensaje(valores) {
...
export function payloadRecordatorio(to, valores) {
...
export function payloadTexto(to, body) {
...
export function enviar(payload, opts) {
  const o = opts || {};
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (o.forzarError) {
        const err = new Error(ERRORES[o.forzarError].titulo);
...
export function estadoVentana(ultimaEntrada, ahora) {
  ...
  const restanteMs = VENTANA_MS - (now - ultimaEntrada);
  return restanteMs > 0
