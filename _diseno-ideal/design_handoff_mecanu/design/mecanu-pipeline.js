...
export const ORDEN_ESTADOS = ESTADOS.map((e) => e.id);
export const ESTADO = {};
...
export const subestadoMeta = (estadoId, subId) => SUBESTADO[`${estadoId}.${subId}`] || null;
export const estadoMeta = (estadoId) => ESTADO[estadoId] || null;

/** Compatibilidad con los componentes que solo necesitan label + kind + paso. */
export const ESTADO_META = ESTADOS.reduce((acc, e) => {
...
export const SUBESTADOS_FRIOS = ESTADOS
  .reduce((acc, e) => acc.concat(e.subestados.filter((s) => s.fueraDelPipeline).map((s) => `${e.id}.${s.id}`)), []);
...
export const PALETA_TAGS = ['#E82B37', '#EC6513', '#419468', '#2D71D7', '#7C4DFF', '#161718'];
...
export const PRESUPUESTO_META = PRESUPUESTO_ESTADOS.reduce((a, e) => { a[e.id] = e; return a; }, {});
export const ORDEN_PRESUPUESTO = PRESUPUESTO_ESTADOS.map((e) => e.id);

/** Estados que cuentan para el badge de la pestaña Campañas (pendientes de acción del taller). */
export const PRESUPUESTO_PENDIENTES = PRESUPUESTO_ESTADOS.filter((e) => e.pendienteTaller).map((e) => e.id);

export const ORIGEN_LINEA = {
...
export const PRESUPUESTO_MODOS = {
...
export const esArrastrable = (estadoId) => !!(ESTADO[estadoId] && ESTADO[estadoId].arrastrable);

/** ¿Esta columna acepta que le suelten una card? */
export const aceptaDrop = (estadoId) => !!(ESTADO[estadoId] && ESTADO[estadoId].aceptaDrop);

/** ¿Se puede editar la propiedad `campo` con la ruta en este estado? */
export function puedeEditar(estadoId, campo) {
...
export const seComunicaAlCliente = (paradaOrigen, paradaDestino) =>
